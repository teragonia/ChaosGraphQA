...
class DirectoryManager:
    ...
    def get_benchmark_path(self, 
        ...
        """
        subdir = self.base_dir / "benchmarks" / generator_type / f"complexity_{complexity}"
    ...
    def get_model_specific_path(self, 
                               model_name: str,
                               subtype: str = "general",
                               filename: Optional[str] = None) -> Path:
        ...
        for generator_dir in benchmarks_dir.iterdir():
            if generator_dir.is_dir():
                ...
                for complexity_dir in generator_dir.iterdir():
                    if complexity_dir.is_dir() and complexity_dir.name.startswith("complexity_"):
                        complexity = complexity_dir.name
                        files = [f.name for f in complexity_dir.iterdir() if f.suffix == ".json"]
            ...
            if benchmark_dir.is_dir():
                ...
                for model_dir in benchmark_dir.iterdir():
                    if model_dir.is_dir():
                        ...
                        files = [f.name for f in model_dir.iterdir() if f.suffix == ".json"]
    ...
    def get_structure_info(self) -> dict:
        ...
        for subdir_name in ["benchmarks", "evaluations", "analysis", "models", "logs", "temp"]:
