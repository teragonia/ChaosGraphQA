...
class MultiHopGenerator(BaseGraphGenerator):
    ...
    def _get_relation_types(self) -> List[str]:
        ...
        base_relations = [
            ...
            "lives_in", "works_at", "owns", "manages",
            "connected_to", "part_of", "member_of", "leads",
            "teaches", "studies_at", "born_in", "created_by"
        ]
    ...
    def generate(self) -> KnowledgeGraph:
        ...
        num_edges = self.rng.randint(
            ...
            self.complexity_params["max_edges"]
        )
    ...
    def _add_structured_paths(self, kg: KnowledgeGraph, entities: Dict[str, Entity]) -> None:
    ...
    def _add_branching_paths(self, kg: KnowledgeGraph, entity_ids: List[str]) -> None:
        ...
        for source_id in list(existing_sources)[:self.complexity_level + 1]:
            available_targets = [eid for eid in entity_ids 
                               if eid != source_id and 
                               not any(r.source == source_id and r.target == eid 
    ...
    def _find_interesting_paths(self, kg: KnowledgeGraph) -> List[Dict[str, Any]]:
        ...
        for start_id in entity_ids[:min(10, len(entity_ids))]:  # Limit to avoid expensive computation
                    ...
                    if nx.has_path(nx_graph, start_id, end_id):
                        ...
                        if 2 <= path_length <= self.max_hops:
                            ...
                            for i in range(len(shortest_path) - 1):
                                edge_data = nx_graph.get_edge_data(shortest_path[i], shortest_path[i + 1])
                                path_relations.append(edge_data.get('relation_type', 'unknown'))
    ...
    def get_path_between(self, kg: KnowledgeGraph, start_id: str, end_id: str) -> Optional[List[str]]:
    ...
    def get_all_paths(
        ...
        try:
            paths = list(nx.all_simple_paths(nx_graph, start_id, end_id, cutoff=max_length))
            return paths
        except (nx.NetworkXNoPath, nx.NodeNotFound):
            return []