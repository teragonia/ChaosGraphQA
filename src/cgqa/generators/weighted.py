...
class WeightedGenerator(BaseGraphGenerator):
    ...
    def __init__(
        ...
        min_edges_for_weights = max(self.complexity_params["min_nodes"] * 2, 
    ...
    def _create_weighted_relationships(self, entities: Dict[str, Entity]) -> List[Relationship]:
        ...
        for _ in range(target_relations):
            ...
            weight = self._calculate_relationship_weight(entity_a, entity_b, relation_config)
    ...
    def _calculate_relationship_weight(
        ...
        if relation_type == "likely_knows" and entity_a.entity_type == "person" and entity_b.entity_type == "person":
            ...
            base_weight = max(0.0, expertise - skill_difficulty * 0.5) + self.rng.uniform(-0.1, 0.1)
            ...
            if entity_a.entity_type == "location" and entity_b.entity_type == "location":
                base_weight = self.rng.uniform(0.0, 1.0)
        ...
        for threshold in self.confidence_thresholds:
            ...
            if len(above_threshold) > 0:
                queries.append({
                    ...
                    "relation_types": list(set(rel.relation_type for rel in above_threshold))
                })
