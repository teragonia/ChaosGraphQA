...
class ConflictingGenerator(BaseGraphGenerator):
    ...
    def __init__(
        ...
        self.conflict_intensity = min(
            0.1 + complexity_level * 0.1, 0.4
        )  # 10-40% conflicting info

        # Need more relationships to create meaningful conflicts
        min_edges_for_conflicts = max(
            self.complexity_params["min_nodes"] * 3, self.complexity_params["min_edges"]
        )
        ...
        kg.metadata.update(
            {
                ...
                "conflict_resolution_strategies": self._suggest_conflict_resolutions(
                    kg
                ),
            }
        )
    ...
    def _choose_appropriate_relation(self, entity_a: Entity, entity_b: Entity) -> str:
        ...
        if entity_a.entity_type == "person" and entity_b.entity_type == "person":
            ...
            entity_a.entity_type == "organization" and entity_b.entity_type == "person"
        ):
    ...
    def _create_transitive_conflicts(
        self, kg: KnowledgeGraph, count: int
    ) -> List[Relationship]:
    ...
    def _create_inheritance_conflicts(
        self, kg: KnowledgeGraph, count: int
    ) -> List[Relationship]:
    ...
    def _create_temporal_conflicts(
        self, kg: KnowledgeGraph, count: int
    ) -> List[Relationship]:
    ...
    def _create_exclusivity_conflicts(
        self, kg: KnowledgeGraph, count: int
    ) -> List[Relationship]:
    ...
    def _find_consistent_subgraphs(self, kg: KnowledgeGraph) -> List[Dict[str, Any]]:
        ...
        consistent_rels = [
            rel for rel in kg.relationships if rel.properties.get("consistent", True)
        ]
