...
class HierarchicalGenerator(BaseGraphGenerator):
    ...
    def __init__(
        ...
        self.branching_factor = [2, 4][
            min(complexity_level - 1, 1)
        ]  # 2-4 children per node
        self.hierarchy_types = self._get_hierarchy_types()

        # Ensure enough entities for meaningful hierarchies
        min_nodes_for_hierarchy = max(
            self.max_depth * self.branching_factor, self.complexity_params["min_nodes"]
        )
    ...
    def generate(self) -> KnowledgeGraph:
            ...
            hierarchy_metadata.append(
                {
                    ...
                    "depth": self._calculate_tree_depth(
                        hierarchy_nodes, hierarchy_edges, root_node
                    ),
        ...
        if current_size < target_size:
            ...
            self._connect_additional_entities(
                kg, list(additional_entities.keys()), all_roots
            )
        ...
        kg.metadata.update(
            {
                ...
                "inheritance_paths": self._find_inheritance_paths(
                    kg, hierarchy_metadata
                ),
            }
        )
    ...
    def _create_hierarchy(
        self, hierarchy_config: Dict[str, Any], prefix: str = "H"
        ...
        num_entities = self.rng.randint(
            self.max_depth * 2, self.max_depth * self.branching_factor
        )
        ...
        for i, name in enumerate(entity_names):
            entity_id = (
                f"{prefix.lower()}_{name.lower()}_{self.rng.randint(1000, 9999)}"
            )
            entity = Entity(
                id=entity_id,
                name=name.title(),
        ...
        while queue and remaining_ids:
            ...
            num_children = min(
                self.rng.randint(1, self.branching_factor), len(remaining_ids)
            )
            ...
            for _ in range(num_children):
                ...
                # Create relationship (child -> parent for all hierarchical relationships)
                # All hierarchical relationships point upward from child to parent:
                ...
                rel = Relationship(
                    source=child_id, target=parent_id, relation_type=relation_type
                )
    ...
    def _add_cross_hierarchy_links(
        self, kg: KnowledgeGraph, hierarchies: List[Dict]
    ) -> None:
    ...
    def _connect_additional_entities(
        self, kg: KnowledgeGraph, additional_entities: List[str], root_nodes: List[str]
    ) -> None:
        ...
        for entity_id in additional_entities:
            ...
            for _ in range(num_connections):
                ...
                rel = Relationship(
                    source=entity_id, target=target_root, relation_type=relation
                )
        ...
        for hierarchy in hierarchies:
            ...
            for rel in kg.relationships:
                if rel.relation_type == relation_type:
                    # All hierarchical relationships now point child -> parent
                    # So we can directly add the edge as-is
            ...
            leaves = [
                n for n in hierarchy_graph.nodes() if hierarchy_graph.in_degree(n) == 0
            ]
