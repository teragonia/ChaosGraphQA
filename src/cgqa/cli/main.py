...
def generate(
    ...
    dir_manager = DirectoryManager(output_dir) if output_dir else get_default_directory_manager()
...
def cleanup(max_age: int, output_dir: Optional[str], dry_run: bool):
    ...
    if not dry_run:
        ...
        if cleaned_count > 0:
            console.print(f"[green]✓[/green] Cleaned up {cleaned_count} temporary files")
        else:
        ...
        if temp_dir.exists():
            ...
            if old_files:
                console.print(f"[yellow]Would clean up {len(old_files)} files:[/yellow]")
