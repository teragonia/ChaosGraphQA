...
class QuestionGenerator:
    ...
    def _register_default_templates(self) -> None:
        ...
        self.template_registry[QuestionType.MULTIHOP] = (
            multihop_templates.get_templates()
        )

        # Hierarchical templates
        hierarchical_templates = HierarchicalTemplates()
        self.template_registry[QuestionType.HIERARCHICAL] = (
            hierarchical_templates.get_templates()
        )

        # Temporal templates
        temporal_templates = TemporalTemplates()
        self.template_registry[QuestionType.TEMPORAL] = (
            temporal_templates.get_templates()
        )

        # Weighted templates
        weighted_templates = WeightedTemplates()
        self.template_registry[QuestionType.WEIGHTED] = (
            weighted_templates.get_templates()
        )

        # Conflicting templates
        conflicting_templates = ConflictingTemplates()
        self.template_registry[QuestionType.CONFLICTING] = (
            conflicting_templates.get_templates()
        )
    ...
    def _generate_question_from_template(
        self, kg: KnowledgeGraph, template: QuestionTemplate, complexity: int
    ) -> Optional[Question]:
    ...
    def _generate_multihop_question(
        ...
        if template.answer_type == AnswerType.PATH:
            # CRITICAL FIX: For "shortest path" questions, find actual shortest path
            # Don't just use metadata path, which might not be the shortest
            if "shortest path" in template.template.lower():
                from ..evaluators.graph_algorithms import GraphAlgorithms

                graph_algorithms = GraphAlgorithms(kg)

                # Get all shortest paths
                shortest_paths = graph_algorithms.find_all_shortest_paths(
                    chosen_path["start"], chosen_path["end"]
                )
                ...
                path_to_use = shortest_paths[0]
            else:
                # Use the chosen path from metadata
                path_to_use = chosen_path["path"]

            path_names = []
            for eid in path_to_use:
            ...
            # Don't just use metadata path, which might not be the shortest
            if (
                "steps does it take" in template.template.lower()
                or "how many steps" in template.template.lower()
            ):
                # This is a step counting question - find the actual shortest path
                actual_shortest_length = self._find_shortest_path_length(
                    kg, chosen_path["start"], chosen_path["end"]
                )
    ...
    def _generate_hierarchical_question(
        ...
        if template.answer_type == AnswerType.BOOLEAN:
            ...
            answer_value = (
                actual_path_length  # Use verified length (depth from leaf to root)
            )
            if actual_path_length > 0:
                explanation = f"The depth of {leaf_entity.name} in the {chosen_path['hierarchy_type']} hierarchy is {answer_value}"
            else:
                explanation = f"{leaf_entity.name} is not part of the {chosen_path['hierarchy_type']} hierarchy (depth: 0)"
        else:
            if actual_path_length > 0:
        ...
        ground_truth = Answer(
            ...
            explanation=explanation,
            metadata={
                "inheritance_path": chosen_path,
                "verified_length": actual_path_length,
            },
        )
    ...
    def _generate_temporal_question(
        ...
        if template.variables.get("sequence_type") == "causal" and causal_chains:
            ...
            if template.answer_type == AnswerType.SINGLE_ENTITY:
                # For "final outcome" questions, find the actual terminal node by traversing the graph
                if "final outcome" in template.template.lower():
                    cause_id = chosen_data["cause"]
                    ...
                    # Find all reachable nodes from cause
                    if cause_id in causal_graph:
                        reachable = nx.descendants(causal_graph, cause_id)

                        # Find terminal nodes (nodes with no outgoing "causes" edges) among reachable
                        terminal_nodes = [
                            node
                        ...
                        if terminal_nodes:
                            ...
                            for terminal in terminal_nodes:
                                try:
                                    path = nx.shortest_path(
                                        causal_graph, cause_id, terminal
                                    )
                                    if len(path) > len(longest_path):
                                        longest_path = path
                                        longest_terminal = terminal
                            ...
                            if longest_terminal:
                                ...
                                answer_value = effect_entity.name
                ...
                # For "how many steps" questions, return the number of edges (length - 1)
                answer_value = int(len(chain_names) - 1)
                explanation = f"Causal chain: {' → '.join(chain_names)}"
            else:
            ...
            if template.answer_type == AnswerType.ENTITY_LIST:
                ...
                # For "last event" questions, find the actual last event by traversing the graph
                if "last event" in template.template.lower():
                    ...
                    # Find all reachable events from the first event
                    if first_event_id in temporal_graph:
                        reachable = nx.descendants(temporal_graph, first_event_id)
                        reachable.add(first_event_id)

                        # Find terminal nodes (nodes with no outgoing edges) among reachable nodes
                        terminal_nodes = [
                            node
                            for node in reachable
                            if temporal_graph.out_degree(node) == 0
                        ]
                        ...
                        if terminal_nodes:
                            ...
                            explanation = (
                                f"The last event in the sequence is {event_names[-1]}"
                            )
                    else:
                        answer_value = event_names[-1]
                        explanation = (
    ...
    def _verify_hierarchical_path(
        self, kg: KnowledgeGraph, path_info: Dict[str, Any]
    ) -> Optional[int]:
        ...
        # - is_a: Cat is_a Mammal (Cat -> Mammal)
    ...
    def _get_actual_hierarchical_path(
        self, kg: KnowledgeGraph, path_info: Dict[str, Any]
    ) -> Optional[List[str]]:
    ...
    def _verify_weighted_path(
        ...
        for rel in kg.relationships:
            if rel.weight is not None:
                ...
                # Add edges following semantic directionality rules
                if RelationshipSemantics.is_bidirectional(rel.relation_type):
                    # Bidirectional relationships allow traversal in both directions
                    G.add_edge(
                        rel.source, rel.target, weight=weight, confidence=confidence
                    )
                    G.add_edge(
                        rel.target, rel.source, weight=weight, confidence=confidence
                    )
                else:
                    # Get the effective semantic direction
                    ...
                    G.add_edge(
                        from_entity, to_entity, weight=weight, confidence=confidence
                    )
    ...
    def _find_shortest_path_length(
        self, kg: KnowledgeGraph, start_entity_id: str, end_entity_id: str
    ) -> Optional[int]:
        """Find the actual shortest path length between two entities respecting semantic directionality."""
        import networkx as nx
        ...
        # Add edges following semantic directionality rules
        for rel in kg.relationships:
            # Use the same semantic directionality logic as the evaluation system
            if RelationshipSemantics.is_bidirectional(rel.relation_type):
                # Bidirectional relationships allow traversal in both directions
                G.add_edge(rel.source, rel.target)
                ...
                # Get the effective semantic direction
                from_entity, to_entity = RelationshipSemantics.get_effective_direction(
                    rel.source, rel.target, rel.relation_type
                )
                G.add_edge(from_entity, to_entity)

        # Find shortest path using directed graph
    ...
    def _generate_weighted_question(
        ...
        if template.variables.get("query_type") == "threshold" and threshold_queries:
            ...
            explanation = f"{answer_value} relationships have weight above {chosen_threshold['threshold']}"
            ...
            if template.answer_type == AnswerType.PATH:
                ...
                if actual_path_info["confidence"] > 0:
                    ...
                    explanation = (
                        f"No path exists between {path_names[0]} and {path_names[-1]}"
                    )
        ...
        context_entity_ids = []
        if (
            template.variables.get("query_type") == "path"
            and weighted_paths
            and "chosen_path" in locals()
        ):
            # For path questions, use the actual entity IDs from the chosen path
            context_entity_ids = [
                chosen_path["path"][0],
                chosen_path["path"][-1],
            ]  # Start and end entity IDs
    ...
    def _generate_conflicting_question(
        ...
        if template.variables.get("query_type") == "detection" and detected_conflicts:
            ...
            variables = {
                ...
                "relation1": chosen_conflict.get(
                    "conflicting_relations", ["relation1", "relation2"]
                )[0],
                "relation2": (
                    chosen_conflict.get(
                        "conflicting_relations", ["relation1", "relation2"]
                    )[1]
                    if len(chosen_conflict.get("conflicting_relations", [])) > 1
                    else "unknown"
                ),
            }
            ...
            template.variables.get("query_type") == "consistency"
            and consistent_subgraphs
        ):
            ...
            if template.answer_type == AnswerType.NUMERIC:
                ...
                for rel in kg.relationships:
                    if rel.properties.get(
                        "consistent", True
                    ):  # Only consider consistent relationships
                        ...
                        if rel.source in question_entities:
                            ...
                            if target_entity and not target_entity.properties.get(
                                "artificial", False
                            ):
                            ...
                            # Only include entities that aren't artificial constructs
                            if source_entity and not source_entity.properties.get(
                                "artificial", False
                            ):
                ...
                if template.answer_type == AnswerType.BOOLEAN:
                    ...
                    entity_subgraph_connected = self._check_entities_connected(
                        kg, question_entities
                    )
        ...
        return Question(
            ...
            context_entities=(
                question_entities
                if "question_entities" in locals()
                else [
                    v
                    for v in [
                        variables.get("source_entity", ""),
                        variables.get("target_entity", ""),
                    ]
                    if v
                ]
            ),
...
class MultiHopTemplates:
    ...
    def get_templates(self) -> List[QuestionTemplate]:
        ...
        return [
            ...
            QuestionTemplate(
                template="What entities lie on the shortest path from {start_entity} to {end_entity}?",
                question_type=QuestionType.MULTIHOP,
                complexity_level=2,
                required_graph_features=["structured_paths"],
                variables={
            ...
            # Hop counting
            QuestionTemplate(
                template="How many steps are in the shortest path from {start_entity} to {end_entity}?",
                question_type=QuestionType.MULTIHOP,
                complexity_level=2,
                required_graph_features=["structured_paths"],
                variables={
    ...
    def get_templates(self) -> List[QuestionTemplate]:
        ...
        return [
            ...
            QuestionTemplate(
                ...
                required_graph_features=["inheritance_paths"],
                variables={
                    "leaf_entity": "Child entity",
                    "root_entity": "Parent entity",
                },
                answer_type=AnswerType.BOOLEAN,
            ),
            QuestionTemplate(
            ...
            QuestionTemplate(
                template="What is the depth of {leaf_entity} in the {hierarchy_type} hierarchy?",
                question_type=QuestionType.HIERARCHICAL,
                complexity_level=2,
                required_graph_features=["inheritance_paths"],
                variables={
...
class WeightedTemplates:
    ...
    def get_templates(self) -> List[QuestionTemplate]:
        ...
        return [
            QuestionTemplate(
                template="How many relationships have weight above {threshold}?",
