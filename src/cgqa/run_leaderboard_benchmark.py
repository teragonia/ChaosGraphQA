...
def run_single_evaluation_with_benchmark(model: str, reasoning_type: str, complexity: int,
                                         run_num: int, benchmark_file: str,
    ...
    output_dir = Path("results") / date_str / model_short / reasoning_type / f"c{complexity}"
    ...
    try:
        subprocess.run(eval_cmd, check=True, capture_output=True, text=True, timeout=timeout)
        ...
        if print_lock:
            with print_lock:
                print(f"      ✗ [{model_short}] Evaluation timed out after {timeout}s (complexity {complexity})")
...
def save_checkpoint(all_results: Dict, completed: int, total: int, start_time: float,
...
def load_checkpoint() -> Dict:
    ...
    try:
        ...
        if (config.get('models') != MODELS or
            config.get('reasoning_types') != REASONING_TYPES or
            config.get('complexity_levels') != COMPLEXITY_LEVELS or
            config.get('num_questions') != NUM_QUESTIONS or
...
def generate_shared_benchmark(reasoning_type: str, complexity: int, run_num: int,
                              num_questions: int, print_lock: Lock) -> str:
    ...
    date_str = datetime.now().strftime("%Y-%m-%d")
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    shared_benchmark_dir = Path("results") / date_str / "_shared_benchmarks" / reasoning_type / f"c{complexity}"
    shared_benchmark_dir.mkdir(parents=True, exist_ok=True)
    benchmark_file = str(shared_benchmark_dir / f"benchmark_run{run_num}_{timestamp}.json")
    ...
    try:
        ...
        with print_lock:
            print(f"      📝 Generated benchmark: {reasoning_type} C{complexity} Run{run_num}")
...
def run_model_tasks(model: str, tasks: List[Dict], benchmark_files: Dict[str, str],
                    completed_tasks: set, print_lock: Lock,
                    results_callback) -> List[dict]:
    ...
    for task in tasks:
        ...
        if get_task_id(task) in completed_tasks:
            with print_lock:
                print(f"      ⏭️  [{model_short}] Skipped: {task['reasoning_type']} C{task['complexity']} Run{task['run_num']}")
            continue
        ...
        result = run_single_evaluation_with_benchmark(
            model, task['reasoning_type'], task['complexity'],
            task['run_num'], benchmark_file, print_lock
        )
    ...
    with print_lock:
        print(f"✅ [{model_short}] Completed all tasks: {len(results)}/{len(tasks)} succeeded")
...
def _main():
    ...
    print(f"   Configs are processed sequentially: {len(REASONING_TYPES)} types × {len(COMPLEXITY_LEVELS)} levels × {NUM_RUNS} runs")
    print(f"   Estimated time: ~{len(REASONING_TYPES) * len(COMPLEXITY_LEVELS) * NUM_RUNS * 2} minutes")
    print("=" * 80)
    ...
    if checkpoint:
        ...
        print(f"   Resuming from {checkpoint['completed']}/{checkpoint['total']} completed")
        print(f"   Previous elapsed time: {checkpoint['elapsed_seconds']/60:.1f} minutes")
    ...
    if not configs_to_run:
        ...
        print(f"   Total tasks: {remaining_configs * len(MODELS)} (all {len(MODELS)} models per config)")
    ...
    for i, config in enumerate(configs_to_run, 1):
        task_key = f"{config['reasoning_type']}_c{config['complexity']}_run{config['run_num']}"
        ...
        print(f"\r   Generated: {i}/{len(configs_to_run)} benchmarks", end='', flush=True)
    ...
    def print_progress_table():
        ...
        with print_lock:
            ...
            print("\n" + "=" * 100)
            print(f"📊 Overall Progress: {completed}/{total_tasks} tasks ({completed/total_tasks*100:.1f}%) | "
            ...
            sorted_models = sorted(MODELS,
                                 key=lambda m: model_progress[m]['completed'] / max(model_progress[m]['total'], 1),
                                 reverse=True)
            ...
            for model in sorted_models:
                ...
                status = "✅ Complete" if prog['completed'] == prog['total'] else "🔄 Running"

                print(f"{model_short:<45} [{bar}] {prog['completed']:>2}/{prog['total']:<2} ({pct:>5.1f}%)  {status}")
    ...
    def handle_result(result_data, task):
        with results_lock:
            ...
            if len(completed_tasks) % CHECKPOINT_INTERVAL == 0:
                save_checkpoint(all_results, len(completed_tasks), total_tasks, start_time, completed_tasks)
    ...
    results_file = str(summary_dir / f"leaderboard_summary_{datetime.now().strftime('%H%M%S')}.json")
