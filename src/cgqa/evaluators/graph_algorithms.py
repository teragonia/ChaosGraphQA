...
class GraphAlgorithms:
    ...
    def find_all_shortest_paths(self, start: str, end: str) -> List[List[str]]:
        ...
        try:
            ...
            # Find all simple paths of that exact length
            all_paths = list(nx.all_simple_paths(
                self.nx_graph, start, end, cutoff=shortest_length
            ))
            
            # Filter to only paths of shortest length
            shortest_paths = [path for path in all_paths if len(path) - 1 == shortest_length]
    ...
    def find_all_simple_paths(
        ...
        try:
            ...
            paths = list(nx.all_simple_paths(
                self.nx_graph, start, end, cutoff=max_length
            ))
            return paths
        except (nx.NetworkXNoPath, nx.NodeNotFound):
            return []
    ...
    def find_paths_with_constraints(
        ...
        filtered_graph = self._create_filtered_graph(allowed_relations, forbidden_relations)
        ...
        try:
            ...
            paths = list(nx.all_simple_paths(
                filtered_graph, start, end, cutoff=max_hops
            ))
            return paths
        except (nx.NetworkXNoPath, nx.NodeNotFound):
            return []
    ...
    def find_connected_components(self) -> List[Set[str]]:
        ...
        return [set(component) for component in nx.connected_components(self.undirected_graph)]
    ...
    def find_cycles(self, max_length: int = 10) -> List[List[str]]:
        ...
        try:
            ...
            pass  # Return empty list if cycle detection fails
