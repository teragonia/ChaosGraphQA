...
class GroundTruthVerifier:
    ...
    def _verify_temporal_answer(self, question: Question) -> Dict[str, Any]:
        ...
        if "causal" in question.question_text.lower() or "triggers" in question.question_text.lower():
            return self._verify_causal_chain_answer(question, causal_chains)
        elif "between" in question.question_text.lower():
            return self._verify_temporal_sequence_intermediate_answer(question, temporal_sequences)
        elif "last event" in question.question_text.lower():
            return self._verify_temporal_sequence_final_answer(question, temporal_sequences, causal_chains)
        else:
            return {
                "is_valid": False,
                "confidence": 0.0,
    ...
    def _verify_causal_chain_answer(self, question: Question, causal_chains: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for chain in causal_chains:
            ...
            if cause_entity and effect_entity:
                if (cause_entity.name.lower() in question_text or 
                    effect_entity.name.lower() in question_text):
                ...
                if effect_entity and effect_entity.name == expected_name:
                    return {
                        ...
                        "details": {
                            ...
                            "verified_chain": [self.kg.get_entity(eid).name for eid in chain["chain"]]
                        }
                    }
    ...
    def _verify_temporal_sequence_intermediate_answer(self, question: Question, temporal_sequences: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for sequence in temporal_sequences:
            if (start_event_name in sequence["event_names"] and 
                end_event_name in sequence["event_names"]):
        ...
        if not relevant_sequence:
            return {
                ...
                "confidence": 0.0,
                "verification_method": "temporal_intermediate_verification",
                "details": {"error": f"No sequence found containing {start_event_name} and {end_event_name}"}
            }
        ...
        if answer.answer_type == AnswerType.ENTITY_LIST:
            ...
            if set(expected_intermediate) == set(answer_list):
                ...
                confidence = len(intersection) / total_expected if total_expected > 0 else 1.0
    ...
    def _verify_temporal_sequence_final_answer(self, question: Question, temporal_sequences: List[Dict[str, Any]], causal_chains: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if sequences_starting_with_event:
            longest_sequence = max(sequences_starting_with_event, key=lambda s: s["length"])
        ...
        if not possible_final_events:
            return {
                ...
                "confidence": 0.0,
                "verification_method": "temporal_final_verification",
                "details": {"error": f"No sequences or chains found starting with {start_event_name}"}
            }
        ...
        if answer.answer_type == AnswerType.SINGLE_ENTITY:
            ...
            # Secondary validation: Check if answer is in any possible final events
                ...
                confidence = 0.8 if longest_sequence_final and expected_name != longest_sequence_final else 1.0
                return {
                    "is_valid": True,
                    "confidence": confidence,
                    "verification_method": "temporal_final_verification",
                    "details": {
                        ...
                        "possible_finals": list(possible_final_events),
                        "starting_event": start_event_name,
                        "note": "Valid ending but may not be from longest sequence" if confidence < 1.0 else None
                    }
                }
            else:
                return {
