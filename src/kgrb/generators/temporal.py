...
class TemporalGenerator(BaseGraphGenerator):
    ...
    def generate(self) -> KnowledgeGraph:
        ...
        self._connect_entities_to_events(kg, list(additional_entities.keys()), [e.id for e in events])
    ...
    def _create_temporal_events(self) -> List[Entity]:
        ...
        event_types = [
            "meeting", "conference", "project", "training", "deadline",
            "launch", "review", "planning", "development", "testing",
            "presentation", "workshop", "milestone", "deployment", "maintenance"
        ]
        
        num_events = self.rng.randint(self.min_events, self.max_events)
        event_names = self.rng.sample(event_types * 3, min(num_events, len(event_types) * 3))
        ...
        for i, event_name in enumerate(event_names):
            ...
            duration_hours = self.rng.choice([
                1, 2, 4, 8,  # Short events (hours)
                24, 48, 72,  # Multi-day events 
                24 * 7, 24 * 14, 24 * 30  # Long projects (weeks/months)
            ])
    ...
    def _create_temporal_relationships(self, events: List[Entity]) -> List[Relationship]:
    ...
    def _add_overlapping_relationships(self, events: List[Entity], relationships: List[Relationship]) -> None:
    ...
    def _add_causal_relationships(self, events: List[Entity], relationships: List[Relationship]) -> None:
    ...
    def _add_containment_relationships(self, events: List[Entity], relationships: List[Relationship]) -> None:
        ...
        for i in range(len(events)):
            for j in range(len(events)):
                ...
                start_container = datetime.fromisoformat(event_container.properties["start_time"])
                end_container = datetime.fromisoformat(event_container.properties["end_time"])
                start_contained = datetime.fromisoformat(event_contained.properties["start_time"])
                end_contained = datetime.fromisoformat(event_contained.properties["end_time"])
                
                # Check if contained event is fully within container event
                if (start_container <= start_contained and 
                    end_contained <= end_container and 
                    event_container.properties["duration_days"] > event_contained.properties["duration_days"]):
    ...
    def _connect_entities_to_events(self, kg: KnowledgeGraph, entity_ids: List[str], event_ids: List[str]) -> None:
        """Connect non-event entities to events."""
        
        participation_relations = ["participates_in", "organizes", "attends", "hosts", "manages"]
